...
EM_JS(void, eagler_journal_copy_bytes,
    ...
    // Resolve HEAPU8 on every call: ALLOW_MEMORY_GROWTH can replace the heap.
...
{
    Clear();
    restoreCopiedBytes_ = 0;
    restoreSkippedBytes_ = 0;
    arenaGrowths_ = 0;
...
bool RollbackJournal::CoversBlock(const FrameRecord &record, const Block &block)
{
    ...
    while (index != NO_BLOCK)
    {
        ...
        if (block.address < candidate.address)
            index = candidate.left;
        else if (block.address - candidate.address >= candidate.size)
            index = candidate.right;
        else
            return block.size <= candidate.size - (block.address - candidate.address);
    }
    return false;
}
